...
class BestPlotCallback(Callback):
    ...
    def setup(self, *args, **kwargs):
        self.widget.layout.title = f"Best {self.metric}="
    ...
    def on_trial_result(self, iteration: int, trials, trial, result, **info):
        with self.widget.batch_update():
            trial_score = None
            meta = [result["trial_id"], f"dataset_fraction: {result.get('dataset_fraction', 1.0)}"]
            meta.extend([f"{k}: {v}" for k, v in result["config"].get(META_KEY, {}).items()])
            text = ", ".join(meta)
            if result:
